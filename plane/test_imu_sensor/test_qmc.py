...
while (time.ticks_ms() - started_at) < duration_ms:
    ...
    # oled.show()
    print("Calibrating... " + str(seconds_remaining) + " seconds remaining.", end="\r")
...
# oled.show()
qmc.calibrate(x_min=min_x, x_max=max_x, y_min=min_y, y_max=max_y, z_min=min_z, z_max=max_z)
print("X min/max: " + str(min_x) + "/" +  str(max_x))
print("Y min/max: " + str(min_y) + "/" +  str(max_y))
print("Z min/max: " + str(min_z) + "/" +  str(max_z))

print("Calibration complete.")

while True:
    print("Heading: " + str(qmc.heading) + " degrees", end="\r")
    time.sleep(0.1)